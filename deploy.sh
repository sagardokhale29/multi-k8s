...
kubectl apply -f k8s
kubectl set image deployments/server-deployment server=sagardokhale29/multi-server:$SHA
kubectl set image deployments/client-deployment client=sagardokhale29/multi-client:$SHA
kubectl set image deployments/worker-deployment worker=sagardokhale29/multi-worker:$SHA